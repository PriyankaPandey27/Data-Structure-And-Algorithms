...
/*Implement_LRU_Cache
A hash is a mechanism by which future requests for that data are served faster
and/or at a lower cost.
Requirements
1. Fixed size: The hash needs to have some bounds to limit memory usage.
2. Fast access: O(1) hash insert and lookup operations.
3. Entry replacement algorithm: When the hash is full, the less useful hash
entries are purged from hash. The algorithm to
replace these entries is Least Recently Used (LRU)
or the hash entries which have not been accessed recently will be replaced.
 */
import java.util.*;
...
class LRUCache {
    ...
    private HashMap<Integer, LRUNode> hash; // HashMap to support O(1) get.
    private LRUNode anchor; // Root of DLL

    public LRUCache(int capacity) {
        this.capacity = capacity;
        this.hash = new HashMap<Integer, LRUNode>();
    ...
    public int get(int key) {
        LRUNode node = this.hash.get(key);
        if (node != null) {
            moveToTop(node);
            return node.value;
        } else {
...
// Remove a key/value node from hash.
        LRUNode node = this.hash.get(key);
